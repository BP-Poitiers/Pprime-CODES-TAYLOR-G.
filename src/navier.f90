...
contains
  ...
  !############################################################################
  !############################################################################
  subroutine pre_correc(ux,uy,uz,ep) ! create u** with the correction on BC
    ...
    ! PERIODIQUE MAIS QUE POUR PIPE FLOW
    !********NCLX==0*****************************************************************************
    !we are in X pencils:
    ...
    ! DIRICHLET ou NEUMANN X --> B-X-u-1/N
    !********NCLX==2********************************************************************************
    !we are in X pencils:
    !------------------------CHANNEL----UNIFORM----ABL----------------------
    if ((itype.eq.itype_channel.or.itype.eq.itype_uniform.or.itype.eq.itype_abl).and.(nclx1==2.and.nclxn==2)) then
       ...
       if ((nrank==0).and.(mod(itime,ilist)==0)) &
         ...
         ! write(*,*) 'Sinus input', real(ux(100, 1, 1), 4)
       do k=1,xsize(3)
          do j=1,xsize(2)
    ...
    ! ########################################################################################################
    ! CASE CAVITY + OUTFLOW --- Utilse mais fonctionne pas !
    ! ########################################################################################################

    if ((itype.eq.itype_strat).and.(ncly1==2.and.nclyn==2).and.(activate_q.eq.1)) then

       !if ((nrank==0).and.(mod(itime,ilist)==0)) then
       !  write(*,*) 'askip je corrige qqch'
       !end if
       ...
       do k=1,xsize(3)
          do j=1,xsize(1)
             ut=ut+byy1(j,k)      ! Out en 0
          enddo
       enddo
       call MPI_ALLREDUCE(MPI_IN_PLACE,ut,1,real_type,MPI_SUM,MPI_COMM_WORLD,code)
       ut=ut/(real(nx*nz,mytype))
       

       do k=1,xsize(3)
          do j=1,xsize(1)
             byy1(j,k)=byy1(j,k)-ut!+ut1
          enddo
       enddo


    endif  ! END ----------- CAVITY IN/OUT --------------------------




   ! ###############################################################################
   ! CAS GENERAUX EN Y - application b_c
   ! ##############################################################################
