...
subroutine parameter(input_i3d)
  ...
  NAMELIST /InOutParam/ irestart, icheckpoint, ioutput, nvisu, ilist, iprocessing, &
       ...
       validation_restart, ampli_sin, pulse_sin, N_file, k_wave, activate_q, centre1, centre2
  ...
  if (nrank==0) then
     ...
     elseif (itype.eq.itype_strat) then
        print *, 'Flow in stratified environment', itype, itype_strat
     elseif (itype.eq.itype_strat_II) then
        print *, 'Flow in stratified environment - hydrostatic contibutions not removed', itype, itype_strat_II   
     else
